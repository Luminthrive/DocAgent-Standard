...
async def init_rerank_service(app: FastAPI):
    rerank_service = RerankService()
    app.state.rerank_service = rerank_service

async def init_query_rewrite_service(app: FastAPI):
    query_rewrite_service = QueryRewriteService()
    app.state.query_rewrite_service = query_rewrite_service

async def init_hybrid_search_service(app: FastAPI):
    hybrid_search_service = HybridSearchService(
        vector_service=app.state.vector_service,
        rerank_service=app.state.rerank_service,
        query_rewrite_service=app.state.query_rewrite_service,
    )
...
async def init_graph(app: FastAPI):
    app.state.tools = create_tools(
        app.state.hybrid_search_service,
...
async def init_services(app: FastAPI):
    ...
    await init_rerank_service(app)
    await init_query_rewrite_service(app)
    await init_hybrid_search_service(app)
...
async def get_rerank_service(request: Request):
    return request.app.state.rerank_service

async def get_query_rewrite_service(request: Request):
    return request.app.state.query_rewrite_service

async def get_hybrid_search_service(request: Request):
